...
void fourteen() {
	...
	for(i=0;i<given+guess;++i) { //generating and storing the binomial coefficients (Pascal's triangle)
		nCr[i][0]=1;
		for(j=1;j<=i;++j)
	...
	for(i=0;i<given;++i) { //calculating and storing differences
		differences[i]=series[0];
